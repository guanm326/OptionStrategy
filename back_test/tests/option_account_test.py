...
def _next():
    ...
    # future.next()
print('开仓')
order = account.create_trade_order(instrument,
                                   LongShort.LONG,
                                   10)
execution_res = instrument.execute_order(order)
account.add_record(execution_res, instrument)
account.daily_accounting(instrument.eval_date)
_next()
print('加仓')
order = account.create_trade_order(instrument,
                                   LongShort.LONG,
