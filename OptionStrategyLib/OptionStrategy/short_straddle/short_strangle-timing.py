...
df_data = df_ivix.reset_index(drop=True).rename(columns={c.Util.PCT_IMPLIED_VOL:'ivix'})
# df_data['iv_htbr'] = df_iv_htbr.reset_index(drop=True)[c.Util.PCT_IMPLIED_VOL]
# df_data.to_csv('iv.csv')
df_iv_stats = df_data[[c.Util.DT_DATE, 'ivix']]
df_iv_stats = filtration(df_iv_stats,'ivix')
