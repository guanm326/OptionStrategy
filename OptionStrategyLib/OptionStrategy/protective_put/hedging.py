...
def open_position(df_index, optionset):
...
def close_position(df_index, strategy_res, optionset):
    dt_date = optionset.eval_date
    dt_maturity = None
...
def bull_spread_vol(df_index, optionset,spot):
    ...
    #                                                                   cd_price=c.CdPriceType.OPEN)
    #     put_long = optionset.select_higher_volume(list_put0)
    std_close = df_index.loc[dt_date, 'std_10']
...
# def shift_bull_spread_vol(option_long, option_short, spot, optionset,df_index,account,unit_index):
#     if option_short is None:
#         [put_long, put_short], name = bull_spread_vol(df_index,optionset)
#         if put_long.strike() != option_long.strike():
#             close_all_options(account)
#         else:
#             strategy_res = {put_short: c.LongShort.SHORT}
...
def shift_bull_spread_vol(option_long,option_short,spot,optionset):
    if option_short is None:
        return False
...
def excute(strategy_res, unit_index, account):
    for option in strategy_res.keys():
        if option is None:
            continue
...
#         # strategy_res = {put_short: c.LongShort.SHORT, put_long: c.LongShort.LONG}
#         [put_long, put_short],name = bull_spread_vol(optionset) # TODO: OPTION STRATEGY
#         strategy_res = {put_short:c.LongShort.SHORT,put_long:c.LongShort.LONG}
