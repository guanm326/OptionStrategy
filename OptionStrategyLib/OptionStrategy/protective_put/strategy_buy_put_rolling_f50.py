...
df_underlying = pd.read_excel('../../../data/fundamental_50.xlsx')
df_underlying['dt_date'] = df_underlying['date'].apply(lambda x: x.date())
df_underlying = df_underlying[df_underlying['dt_date']>=datetime.date(2015,1,1)].reset_index(drop=True)
df_underlying[c.Util.ID_INSTRUMENT] = 'fundamental50'
...
while d2 <= end_date:
    ...
    analysis['option_premium'] = total_premium/init_fund
    df_underlying_1.loc[:,'npv_50etf'] = df_underlying_1.loc[:,c.Util.AMT_CLOSE]/df_underlying_1.loc[0,c.Util.AMT_CLOSE]
    ...
    d2 = d1 + datetime.timedelta(days=365)
print(df)
df.to_csv('../../accounts_data/buy_put_rolling-foundamental50-m='+str(moneyness)+'.csv')
